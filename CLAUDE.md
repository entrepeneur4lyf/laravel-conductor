...
- **Read `src/Engine/Supervisor.php` before touching any decision logic.** It is the largest and most opinionated file in the package and the test suite (`SupervisorRemediationTest`, `RetryAndDispositionTest`, `EscalationEvaluatorTest`) pins its behavior tightly.
- **Definition fields that are accepted but not yet executed by the runtime:** `parallel`, `foreach`, `tools`, `provider_tools`, `on_fail` transition semantics, `defaults.timeout` enforcement. They validate and serialize correctly, but adding tests that assume runtime fan-out, tool invocation, or `on_fail` consumption will fail. The README's "What It Does Not Yet Do" list is the source of truth here — keep it in sync if you implement any of those.
