...
- **Definition fields that are accepted but not yet executed by the runtime:** `parallel` and `foreach` (deferred to F11). They validate and serialize correctly, but adding tests that assume runtime fan-out will fail. The README's "What It Does Not Yet Do" list is the source of truth — keep it in sync if you implement F11.
- **Per-step `timeout` is enforced as a per-call HTTP deadline.** `RunProcessor::buildStepInput()` forwards `$stepDefinition->timeout` into `StepInputData::$meta['timeout']`, and `AtlasStepExecutor::execute()` calls `$request->withTimeout($timeout)` when the value is positive. The deadline applies to the Atlas LLM round-trip only — in-process work in the same step is not bounded by this value. Step timeouts can be inherited from `defaults.timeout` (see the defaults merge entry above).
- **`on_fail` is consumed as a fallback transition.** When a step fails, `Supervisor::handleFailure()` runs the failure handler cascade, then escalation (if retry budget remains), then — if no handler resolved the failure and escalation said `fail` (or there was no escalation) — routes to `$stepDefinition->on_fail` via `Supervisor::failOrTransitionOnFail()`. The target may be any known step or any of the terminal targets (`complete`, `discard`, `fail`, `cancel`). A handler that matches and resolves the failure (e.g. `skip`) bypasses on_fail entirely. If no on_fail target is declared, the run fails.
- **Workflow `defaults` are merged into individual steps at load time** by `YamlWorkflowDefinitionRepository::applyDefaults()`. Only `retries`, `timeout`, `tools`, `provider_tools`, and `meta` are merge-eligible. Step values always win over defaults (the merge uses `array_key_exists` on the raw payload, so explicit step values — including explicit `0` or empty arrays — are preserved). Step-identity fields (`id`, `agent`, `prompt_template`, `output_schema`) are rejected from the defaults block with an `InvalidArgumentException` at load time.
