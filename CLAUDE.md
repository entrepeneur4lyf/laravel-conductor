...
- **Definition fields that are accepted but not yet executed by the runtime:** `parallel`, `foreach`, and per-step `timeout` enforcement at execution time. They validate and serialize correctly, but adding tests that assume runtime fan-out will fail. The README's "What It Does Not Yet Do" list is the source of truth — keep it in sync if you implement any of those.
